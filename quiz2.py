...
class ScoreboardDrawer:
    ...
    def generate_image(self, players, group_idx, question_text, answer_text, timer_str="00:00", timer_alert=False, mode="2R", semi_set_idx=1, sf_hide_scores=False):
        ...
        header = ""
        if mode == "2R": header = f"2nd Round group{group_idx + 1}"
        elif mode.startswith("3rd"): header = "3rd Round course select"
        elif mode == "SEMI": header = f"Semifinal Nine Hundred - Set {semi_set_idx}"
        elif mode == "FINAL": header = "Final - Triple Seven"
        elif mode == "EXTRA": header = "Extra Round 2nd step"
        elif mode == "SF_FOLLOW": header = "SF Follow-up"
        else: header = f"3rd Round {mode}"
        ...
        if mode in ["10by10", "Swedish10", "Freeze10", "10up-down"]:
        ...
        display_players = []
        if mode == "2R": display_players = players[:12]
        elif mode == "SEMI": display_players = players[:9]
        elif mode == "FINAL": display_players = players[:3]
        elif mode == "EXTRA": display_players = players[:12]
        else: display_players = players[:5]
...
class QuizApp(tk.Tk):
    ...
    def update_preview_image(self):
        ...
        if self.mode == "3RD":
            ...
            pil_img = self.drawer.generate_image(players, self.current_group_idx, q, a, timer_str=t_str, timer_alert=t_alert, mode="2R" if self.mode=="SCORE" or isinstance(self.mode, int) else self.mode, semi_set_idx=self.semi_set_idx, sf_hide_scores=self.sf_hide_scores)
